...
final class CreateRobotsLogTable extends AbstractMigration
{
    public function change(): void
    {
        $environment = $this->input->getParameterOption('-e');
        if ("robots_log" === $environment) {
            ...
            $this->execute("CREATE TABLE IF NOT EXISTS `robotslog` (
                  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'Ид записи',
                  `robot` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Ид робота (текстовое представление названия робота)',
                  `timestamp` datetime DEFAULT CURRENT_TIMESTAMP COMMENT 'Дата события',
