...
final class AddSponsoredBrandsVideoStatTable extends AbstractMigration
{
    public function change(): void
    {
        $environment = $this->input->getParameterOption('-e');
        if (in_array($environment, ["production", "development", "testing"])) {
            $this->execute("CREATE TABLE IF NOT EXISTS `sponsored_brands_video_ads_stat` (
            ...
            // Добавим индексы
            $table = $this->table('sponsored_brands_video_ads_stat');
            $table->addIndex(['asin', 'sku', 'date', 'campaignId'], ['unique' => true,'name' => 'productStatKey'])->save();
        }
    }
}
